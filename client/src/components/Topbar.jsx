...
export default function Topbar() {
  return (
    <div className="fixed top-6 left-0 right-0 z-50 flex justify-center px-4">
      <nav className="retro-card !rounded-full !py-3 !px-8 flex items-center w-full max-w-4xl shadow-2xl">
        <ul className="flex items-center justify-between w-full font-['VT323'] uppercase text-xl md:text-2xl text-white">
          {navItems.map((item) => (
            <li key={item.name}>
              <NavLink
                to={item.path}
                className={({ isActive }) =>
                  `px-3 py-1 transition-all duration-300 hover:text-[var(--color-cyber-primary)] hover:drop-shadow-[0_0_8px_var(--color-cyber-primary)] ${
